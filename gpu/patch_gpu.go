...
func prove(spr *cs.SparseR1CS, pk *ProvingKey, fullWitness witness.Witness, opts ...backend.ProverOption) (*plonkbls12381.Proof, error) {
	...
	// solve constraints
	g.Go(func() error {
		nvtx.RangePush("prove: solveConstraints")
		defer nvtx.RangePop()
	...
	// complete qk
	g.Go(func() error {
		nvtx.RangePush("prove: completeQk")
		defer nvtx.RangePop()
	...
	g.Go(func() error {
		nvtx.RangePush("prove: initBlindingPolynomials")
		defer nvtx.RangePop()
	...
	g.Go(func() error {
		nvtx.RangePush("prove: deriveGammaAndBeta")
		defer nvtx.RangePop()
	...
	g.Go(func() error {
		nvtx.RangePush("prove: buildRatioCopyConstraint")
		defer nvtx.RangePop()
	...
	// compute h
	g.Go(func() error {
		nvtx.RangePush("prove: computeQuotient")
		defer nvtx.RangePop()
	...
	g.Go(func() error {
		nvtx.RangePush("prove: computeLinearizedPolynomial")
		defer nvtx.RangePop()
	...
	// Batch opening
	g.Go(func() error {
		nvtx.RangePush("prove: batchOpening")
		defer nvtx.RangePop()
	...
	if useGPU {
		...
		icicle_runtime.RunOnDevice(&s.pk.deviceInfo.Device, func(args ...any) {
			...
			for i := 0; i < len(s.x); i++ {
				...
				if uploadedCount <= 3 || uploadedCount == polysToUpload {
		...
		batchApply(s.x, func(p *iop.Polynomial) {
			...
			if useGPU {
				if idx, ok := poly2idx[p]; ok {
					// NVTX 标记：batchApply 中调用 toCosetLagrangeOnGPUorCPU_DEV (GPU 路径)
					nvtx.RangePush(fmt.Sprintf("batchApply: toCosetLagrangeOnGPUorCPU_DEV (poly idx=%d, %d coeffs)", idx, len(coeffs)))
			...
			// NVTX 标记：batchApply 中调用 toCosetLagrangeOnGPUorCPU_DEV (CPU 回退路径)
			nvtx.RangePush(fmt.Sprintf("batchApply: toCosetLagrangeOnGPUorCPU_DEV (CPU fallback, %d coeffs)", len(coeffs)))
...
func (s *instance) toCosetLagrangeOnGPUorCPU_DEV(
	...
	if HasIcicle && s.pk != nil && s.pk.deviceInfo != nil && xdev != nil {
		...
		icicle_runtime.RunOnDevice(&s.pk.deviceInfo.Device, func(args ...any) {
			...
			startStep := time.Now()
			nvtx.RangePush(fmt.Sprintf("MontConv: dev->nonMont (poly[%d coeffs])", len(coeffs)))
			...
			nvtx.RangePop()
			elapsed := time.Since(startStep)
			fmt.Printf("		[Timing] MontConv(dev->nonMont) 耗时: %.6f ms\n", float64(elapsed.Nanoseconds())/1e6)

			startStep = time.Now()
			nvtx.RangePush(fmt.Sprintf("VecMulOnDevice: poly[%d coeffs]", len(coeffs)))
			...
			nvtx.RangePop()
			elapsed = time.Since(startStep)
			fmt.Printf("		[Timing] VecMulOnDevice 耗时: %.6f ms\n", float64(elapsed.Nanoseconds())/1e6)

			startStep = time.Now()
			nvtx.RangePush(fmt.Sprintf("MontConv: dev->Mont (poly[%d coeffs])", len(coeffs)))
			...
			nvtx.RangePop()
			elapsed = time.Since(startStep)
			fmt.Printf("		[Timing] MontConv(dev->Mont) 耗时: %.6f ms\n", float64(elapsed.Nanoseconds())/1e6)

			// 正变换 NTT：Canonical -> Lagrange(小域)
			startStep = time.Now()
			nvtx.RangePush(fmt.Sprintf("NttOnDevice: poly[%d coeffs]", len(coeffs)))
			...
			nvtx.RangePop()
			elapsed = time.Since(startStep)
			fmt.Printf("		[Timing] NttOnDevice 耗时: %.6f ms\n", float64(elapsed.Nanoseconds())/1e6)
			...
			nvtx.RangePop()
			elapsed = time.Since(startStep)
			fmt.Printf("		[Timing] CopyFromDevice (D2H) 耗时: %.6f ms\n", float64(elapsed.Nanoseconds())/1e6)
			fmt.Printf("		[Memory] After CopyFromDevice (D2H):\n")
			printMemoryInfo("After D2H")

			// 4) 立刻把 dev 恢复为 Canonical，方便下一个 coset 继续复用
			startStep = time.Now()
			nvtx.RangePush(fmt.Sprintf("INttOnDevice: poly[%d coeffs] (restore canonical)", len(coeffs)))
			...
			nvtx.RangePop()
			elapsed = time.Since(startStep)
			fmt.Printf("		[Timing] INttOnDevice (restore canonical) 耗时: %.6f ms\n", float64(elapsed.Nanoseconds())/1e6)
		})
		<-done
